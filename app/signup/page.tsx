    ...
    try {
      const { error: signUpError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            full_name: fullName,
            // By default, everyone signs up as an employee. An admin can change this later.
            role: 'employee',
          }
        }
      })
      ...
      setError(err?.message || (typeof err === 'object' ? JSON.stringify(err, null, 2) : String(err)) || 'Failed to sign up. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-4">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">Join your organization</p>
        </div>
