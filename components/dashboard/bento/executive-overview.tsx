...
export function ExecutiveOverview({
  ...
  return (
    <WidgetShell
      ...
      noPadding
    >
      <div className="relative h-full flex flex-col p-4 md:p-6 lg:p-8 overflow-hidden">
        ...
        <div className="relative z-10 flex items-start justify-between mb-4 md:mb-6">
          <div>
            <h1 className="text-xl md:text-2xl lg:text-3xl font-bold tracking-tight">{greeting}</h1>
            <p className="text-xs md:text-sm text-muted-foreground mt-0.5">{date}</p>
          </div>
          <div className="flex items-center gap-2 px-2.5 py-1 md:px-3 md:py-1.5 rounded-full bg-primary/8 border border-primary/15">
            <Sparkles className="w-3 h-3 md:w-3.5 md:h-3.5 text-primary" />
            <span className="text-[10px] md:text-[11px] font-semibold text-primary">AI Active</span>
            <StatusIndicator variant="online" size="sm" />
          </div>
        </div>

        {/* Main Content — Health Score + Stats */}
        <div className="relative z-10 flex-1 flex flex-col gap-4 md:gap-6">
          {/* Mobile: Horizontal progress bar for Org Health */}
          <div className="md:hidden">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Org Health</span>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold tabular-nums w-12 text-right">{healthScore}%</span>
            </div>
          </div>

          {/* Desktop/Tablet: Radial Progress */}
          <div className="hidden md:flex flex-row gap-6 lg:gap-10 items-center">
          ...
          <div className="grid grid-cols-2 gap-2.5 md:hidden">
            {miniStats.map((stat, i) => (
              <motion.div
                ...
                className="flex flex-col gap-1 p-2.5 rounded-xl bg-surface-secondary/60"
              >
                <div className="flex items-center gap-1.5">
                  <span className="text-muted-foreground">{stat.icon}</span>
                  <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider truncate">
        ...
        <div className="relative z-10 mt-3 md:mt-4 flex items-center gap-3 pt-3 md:pt-4 border-t border-border/30">
          <Brain className="w-3.5 h-3.5 md:w-4 md:h-4 text-primary/60" />
          <span className="text-[10px] md:text-[11px] text-muted-foreground font-medium">AI Confidence</span>
          ...
          <span className="text-[10px] md:text-[11px] font-semibold tabular-nums">{aiConfidence}%</span>
        </div>
      </div>
    </WidgetShell>
  )
