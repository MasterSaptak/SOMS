...
import { addEmployeeCertificationAction, deleteEmployeeCertificationAction } from '@/app/actions/employee.actions'
import { useRouter } from 'next/navigation'

export function CertificationsTab({ employeeId, canEdit, initialData }: { employeeId: string, canEdit: boolean, initialData: any[] }) {
  ...
  return (
    <div className="bg-card border border-border/50 rounded-xl p-6 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {initialData?.map(c => (
          <div key={c.id} className="border border-border/50 rounded-lg p-4 flex justify-between items-start group">
            ...
            {canEdit && (
              <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity hover:text-red-500 shrink-0 ml-2" onClick={() => handleDelete(c.id)}>
