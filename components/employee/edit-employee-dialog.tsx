...
import type { Employee, Department, Designation, WorkLocation } from '@/lib/types'
import { updateEmployeeAction, getEmployeesAction } from '@/app/actions/employee.actions'
import { getDepartmentsAction, getDesignationsAction, getWorkLocationsAction } from '@/app/actions/structure.actions'
...
export function EditEmployeeDialog({ open, onOpenChange, employee }: EditEmployeeDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      getDepartmentsAction(orgId).then(r => r.success && setDepartments(r.data))
      getDesignationsAction(orgId).then(r => r.success && setDesignations(r.data))
      getWorkLocationsAction(orgId).then(r => r.success && setLocations(r.data))
      getEmployeesAction(orgId).then(r => r.success && setManagers(r.data))
    }
  }, [open, employee.organizationId])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        alert('Failed to save changes. Make sure you are using a real Demo Organization.')
      }
    } finally {
      setIsSaving(false)
    }
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.managerId} onValueChange={v => updateForm('managerId', v)}>
                ...
                <SelectContent>
                  ...
                  {managers.map(e => <SelectItem key={e.id} value={e.id}>{e.firstName} {e.lastName}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
