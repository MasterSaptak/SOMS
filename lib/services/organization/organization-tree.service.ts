...
export class OrganizationTreeService {
  async getFullTree(orgId: string): Promise<Result<any>> {
    try {
      ...
      // 1. Fetch branches
      const { data: branches, error: err1 } = await (client as any)
        .from('branches')
        .select('*')
      ...
      // 2. Fetch departments
      const { data: departments, error: err2 } = await (client as any)
        .from('departments')
      ...
      const { data: teams, error: err3 } = await (client as any)
        .from('teams')
      ...
      // Compose the tree
      const tree = branches.map((branch: any) => {
        const branchDepts = departments.filter((d: any) => d.branch_id === branch.id)
        
        const deptsWithTeams = branchDepts.map((dept: any) => {
